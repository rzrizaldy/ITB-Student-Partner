...
//Inisialisasi latitude dan longitude terdekat
	  var latd = parseFloat(array1[1]);
	  var lngd = parseFloat(array2[1]);

//Mencari Jarak FotoCopy terdekat
	  for (i = 0; i < array1.length; i++) { 
	  if (Math.sqrt(Math.abs(parseFloat(array1[i])-lat)*Math.abs(parseFloat(array1[i])-lat)+
					Math.abs(parseFloat(array2[i])-lng)*Math.abs(parseFloat(array2[i])-lng))
		  <
		  Math.sqrt((	Math.abs(latd-lat)*Math.abs(latd-lat)+
						Math.abs(lngd-lng)*Math.abs(lngd-lng))
	  	  )
		  )
		  
	  {
		  latd=parseFloat(array1[i]);
		  lngd=parseFloat(array2[i]);
	  }
	  }
//Marker untuk menunjukkan lokasi FotoCopy Terdekat
	  map_marker = new google.maps.Marker({position: {lat: latd, lng: lngd}, map: map});
	  }
